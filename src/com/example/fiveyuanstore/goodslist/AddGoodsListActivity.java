...
public class AddGoodsListActivity extends Activity {
	...
	void showList(){
		...
		AlertDialog . Builder builder = new AlertDialog.Builder(AddGoodsListActivity.this);
	...
	void add() {
		...
		Request request = Server.requestBuilderWithPath("/addGoodsList").method("post", null).post(builder.build())
				.build();
		
		final ProgressDialog progressD = new ProgressDialog(AddGoodsListActivity.this);
	 			...
	 			runOnUiThread(new Runnable() {
	 				...
	 				public void run() {
	 					 progressD.dismiss();
	 						...
	 						new AlertDialog.Builder(AddGoodsListActivity.this)
			...
			public void onResponse(Call arg0, final Response arg1) throws IOException {
				try {
					...
					runOnUiThread(new Runnable() {
						...
						@Override
						public void run() {
							AddGoodsListActivity.this.data = data.getContent();
